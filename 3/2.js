...
const file = input.readFileSync(path.join(__dirname, 'input2.txt'), 'utf8').trim();
...
const stringOfMuls = file.matchAll(/mul\([0-9]+,[0-9]+\)/ig);
...
for (const match of stringOfMuls) {
  ...
  const numbersString = match[0].matchAll(/[0-9]+/ig);
